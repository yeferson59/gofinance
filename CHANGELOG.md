...
- `decimal.Decimal.UnmarshalJSON` now accepts a JSON string holding a number (`"9.99"`) as well as a bare number. It is not what `MarshalJSON` writes, but it is what a producer sends to protect precision from readers that treat every JSON number as a `float64`, and it is the form a `Decimal` takes as a map key — refusing it left the package unable to read documents it can write. The digits go through the same strict parser `NewFromString` uses, so a quoted value is held to the same standard as an unquoted one: exponent notation, padding and more than 19 fractional digits are still rejected
- `money.Money.UnmarshalJSON` now accepts a bare JSON string holding an amount (`"123.45"`) as well as a bare number, both read as USD — the default `Scan` already applied. Within the object form the `currency` member also accepts the integer form `money.Currency` has always accepted on its own
- **Breaking**: `money.Currency.MarshalJSON` returns an error for an unrecognised currency instead of encoding it as `"Currency(255)"`, the form `String` produces for people reading output. No decoder could read that back, so it wrote a document this package could not parse. `Money.MarshalJSON` already reported the error, and `MarshalText` now does too
...
- `money.Currency` and `decimal.Decimal` implement `encoding.TextMarshaler`, `encoding.TextUnmarshaler` and `encoding.TextAppender`, so both types encode and decode in formats that carry text rather than JSON — YAML, TOML, XML, `flag.TextVar`, `log/slog` — and in `encoding/json` v1 map keys. A currency writes its canonical uppercase ISO code and reads back case-insensitively with surrounding space trimmed, as `Scan` already did; a decimal writes exactly what `String` prints and parses with the same strict parser as `NewFromString`, so exponent notation and padded input are rejected rather than silently reinterpreted. An unrecognised currency is an error rather than the `Currency(<n>)` form `String` uses for humans. JSON output is unchanged: `encoding/json/v2` prefers the existing `MarshalJSON`/`UnmarshalJSON` for values and map keys alike
- `decimal.Decimal` implements `json.MarshalerTo` and `json.UnmarshalerFrom`, the streaming interfaces `encoding/json/v2` prefers over `MarshalJSON`/`UnmarshalJSON`. They encode through the encoder's own buffer rather than returning a fresh slice (one allocation less per value), and they close the gap that made a `Decimal` unusable as a JSON map key: an object member name must be a string, so the bare number `MarshalJSON` returns was rejected outright by the encoder. `map[decimal.Decimal]T` now encodes as `{"1.5":…}` and decodes back. The `json:",string"` struct tag is honoured too, via the `StringifyNumbers` option — producers use it so a decimal survives readers that parse every JSON number as a `float64`. Values in every other position are unchanged: still bare JSON numbers
- `money.Currency` and `money.Money` implement `json.MarshalerTo` and `json.UnmarshalerFrom` too. Both build their JSON directly into the encoder's buffer instead of going through an intermediate struct and the reflection-based encoder, and both decode member by member rather than into one. On an Apple M1: encoding a currency 249 → 115 ns/op and 6 → 2 allocations, encoding an amount 481 → 293 ns/op and 10 → 6, decoding an amount through `encoding/json/v2` 913 → 251 ns/op and 17 → 2. The wire format is unchanged in both directions
- `decimal.Decimal`, `money.Currency` and `money.Money` implement `encoding.BinaryMarshaler`, `encoding.BinaryUnmarshaler` and `encoding.BinaryAppender`. This is what makes them reachable by binary codecs: `encoding/gob` consults `GobEncoder` and `BinaryMarshaler` and never falls back to `MarshalText`, so a `Decimal` or a `Money` — whose fields are all unexported — previously failed to encode at all with `gob: type decimal.Decimal has no exported fields`. The same interface is what lets an amount be handed straight to a cache client such as go-redis. The layouts are versioned, because unlike a document a binary blob is persisted and may be read back by a build whose internals have moved: a `Decimal` is a version byte, a header byte holding the sign and scale, and the 128-bit coefficient big-endian (18 bytes); a `Currency` is a version byte and the ISO 4217 code (4 bytes) — the code rather than the integer behind the constant, which is this package's own numbering and would change meaning if the list were ever renumbered; a `Money` is a version byte, the ISO code, then the amount in the decimal package's own form (22 bytes). Zero has exactly one encoding, so equal values encode to equal bytes and a blob can be compared or used as a cache key. Encoding a decimal costs 2 ns/op and no allocations against 73 ns/op and four for the text form, but the binary form is not the compact one: a 128-bit coefficient costs sixteen bytes whatever digits it holds, so ordinary amounts are smaller as text
...
- `investment.IRR` documents what a series with more than one sign change means for the result: several rates can zero the net present value, and it returns one of them without signalling the ambiguity. That is the standard behaviour of the measure rather than a limitation of this implementation, and the documentation now says so and points to `NPV` at a chosen discount rate, which has one answer by construction
...
- **Breaking**: `tvm.SolveN` returned a negative number of periods when the payment pointed the same way as the balance — money going out on both sides never repays anything, but the equation still has a root at a term in the past. A term runs forwards, so that case now returns `ErrNoSolution`. A term of zero is still valid: it means the target is already met
- **Breaking**: `annuities.BuildSchedule` truncated a fractional term silently, so a request for 5.7 periods produced five rows. A schedule has one row per payment, so a fractional term is now rejected with `ErrInvalidPeriods` rather than answering a different question than the one asked. Round or truncate the term yourself if that is what you want
...
- `money.Currency.UnmarshalJSON` silently decoded to `XXX` — "no currency" — for any JSON value that was neither a string nor a number. `true`, `null` and `[1]` all left the receiver set and returned no error. It also read a number wider than the type by truncating it into range, so `256` decoded as currency 0, which is `XXX` again; the number form is now parsed at the width of the type. Both decoders also reject trailing content after the value, the same defect fixed in `decimal`
- The trailing-content checks trim what JSON calls whitespace — space, tab, carriage return, newline — rather than what `bytes.TrimSpace` does. A vertical tab, a form feed or a non-breaking space after a value is content, and accepting it made these decoders read documents `encoding/json/v2` rejects. Found by a fuzz target that cross-checks the v1 and v2 decoding paths against each other
- `decimal.Decimal.UnmarshalJSON` accepted trailing content after the value it read, so `"00"` — two JSON numbers — decoded as zero instead of failing. `json.Unmarshal` never passes such input, but this method is also called directly, including by `money`'s own decoder. Found by a new fuzz target that cross-checks the v1 and v2 decoding paths against each other; its corpus entry is kept as a regression
