...
var (
	...
	rateSolverMaxGrowth = decimal.MustFromFloat64(1e15)
	rateSolverTolerance = decimal.MustFromFloat64(1e-12)
	rateSolverTwo       = decimal.MustFromFloat64(2)
	...
	overflowSentinel = decimal.MustFromFloat64(1e18)
)
...
func rateBounds(periods decimal.Decimal) (decimal.Decimal, decimal.Decimal, error) {
	reciprocal, err := decimal.One.Div(periods)
	if err != nil {
	...
	return lowerGrowth.Sub(decimal.One), upperGrowth.Sub(decimal.One), nil
}
...
func safePow(base, exponent decimal.Decimal) (decimal.Decimal, error) {
...
func solveRate(target, periods decimal.Decimal, pv func(rate decimal.Decimal) (decimal.Decimal, error)) (decimal.Decimal, error) {
...
func presentValueOrdinary(payment, rate, periods decimal.Decimal) (decimal.Decimal, error) {
	if rate.IsZero() {
		return payment.Mul(periods), nil
	}

	growthFactor := rate.Add(decimal.One)
	...
	numerator := payment.Mul(decimal.One.Sub(discountFactor))
...
// ordinary annuity (payments at the end of each period).
func futureValueOrdinary(payment, rate, periods decimal.Decimal) (decimal.Decimal, error) {
	if rate.IsZero() {
		return payment.Mul(periods), nil
	}
	...
	numerator := payment.Mul(growthPower.Sub(decimal.One))
...
func presentValueDue(payment, rate, periods decimal.Decimal) (decimal.Decimal, error) {
	ordinary, err := presentValueOrdinary(payment, rate, periods)
	if err != nil {
		return decimal.Decimal{}, err
	}

	return ordinary.Mul(rate.Add(decimal.One)), nil
}

// futureValueDue is futureValueOrdinary × (1+i): the future value of an
// annuity due (payments at the beginning of each period).
func futureValueDue(payment, rate, periods decimal.Decimal) (decimal.Decimal, error) {
