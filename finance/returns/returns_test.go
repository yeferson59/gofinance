...
func TestCAGR(t *testing.T) {
	...
	cagr, err := CAGR(usd(1000), usd(2000), money.MustFromFloat64(5))
...
func TestCAGRDecline(t *testing.T) {
	...
	cagr, err := CAGR(usd(1000), usd(500), money.MustFromFloat64(2))
	require.NoError(t, err)
	assert.InDelta(t, -0.292893, cagr.InexactFloat64(), 1e-6)
}

func TestCAGRErrors(t *testing.T) {
	_, err := CAGR(usd(1000), money.MustMoneyFromFloat64(2000, money.EUR), money.MustFromFloat64(5))
	assert.ErrorIs(t, err, money.ErrCurrencyMismatch)

	_, err = CAGR(usd(0), usd(2000), money.MustFromFloat64(5))
	assert.ErrorIs(t, err, ErrNonPositiveValue)

	_, err = CAGR(usd(1000), usd(2000), money.MustFromFloat64(0))
...
func TestAnnualized(t *testing.T) {
	...
	a, err := Annualized(money.MustFromFloat64(0.20), money.MustFromFloat64(3))
	require.NoError(t, err)
	assert.InDelta(t, 0.062659, a.InexactFloat64(), 1e-6)
}

func TestAnnualizedErrors(t *testing.T) {
	_, err := Annualized(money.MustFromFloat64(0.20), money.MustFromFloat64(0))
	assert.ErrorIs(t, err, ErrNonPositivePeriods)

	// Losing more than everything is undefined.
	_, err = Annualized(money.MustFromFloat64(-1.5), money.MustFromFloat64(3))
	assert.ErrorIs(t, err, ErrNonPositiveValue)
}

func TestMustHelpers(t *testing.T) {
	assert.InDelta(t, 0.25, MustROI(usd(1000), usd(1250)).InexactFloat64(), 1e-9)
	assert.Panics(t, func() { MustCAGR(usd(0), usd(1), money.MustFromFloat64(1)) })
}
